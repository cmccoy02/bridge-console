...
async function migrate() {
  ...
  try {
    ...
    console.log(repoTable ? '[OK] Exists' : '[X] Missing');

    console.log('\nScans table:');
    console.log(scansTable ? '[OK] Exists' : '[X] Missing');

    if (scansTable) {
      // Check if repositoryId column exists
      const hasRepoId = scansTable.sql.includes('repositoryId');
      console.log('Has repositoryId column:', hasRepoId ? '[OK] Yes' : '[X] No');

      if (!hasRepoId) {
        console.log('\n[WARN] Schema is outdated!');
    ...
    console.log('\n[DONE] Database schema is correct!\n');

  } catch (error) {
    console.error('[FAIL] Migration check failed:', error.message);
